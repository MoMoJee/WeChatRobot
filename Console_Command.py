...
def Console_Command(logger, message, folder_path_History, client, wx = "None", role = 0):
    ...
    if "清理内存" in message:
        ...
        return Role.return_role_words(logger, role_key=role, role_code="2000")
        ...
        if continuous_numbers:
            global_state.conversation_History = History.delete_n_messages(logger, global_state.conversation_History, int(continuous_numbers[0]))#这里我们取第一个
        ...
        global_state.conversation_History = History.clear_n_percent_of_history(logger, global_state.conversation_History, 100)  # 调用清理函数，删除全部聊天数据
        ...
        # 保存对话历史到文件。
        History.save_conversation_history_to_file(logger, global_state.conversation_History, folder_path_History)
        return Role.return_role_words(logger, role_key=role, role_code="2001")
    elif "日志" in message:
        ...
        return Role.return_role_words(logger, role_key=role, role_code="2002")
    elif "好好说话" in message:
        logger.info("【Console】请求：重申初始设定")
        return Chatting.chat_with_AI(logger, Role.return_role_words(logger, role_key=role, role_code="2003"), client,"system", role=role)
    elif "余额" in message:
        logger.info("【Console】请求余额查询")
        balance = Chatting.get_api_balance(logger, 'sk-m8SUBSaNEAwYWuEnqhL4loUTd6v96EPAUBVuNXT5eKnVzZYL')
